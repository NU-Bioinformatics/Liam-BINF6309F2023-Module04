...
# differential expression analysis using DESeq2 of salmon quantified Aiptasia genome alignment
...
# merge other table info - by index
ko <- read_table('ko.txt', col_names = c('other', 'KO'))
dfAll <- dfAll %>% mutate(KO = rownames(.))
dfAll <- left_join(dfAll, ko, by = c('KO'))
colnames(tx2gene)[2] <- 'KO'
dfAll <- left_join(dfAll, tx2gene, by = c('KO'))
...
write.csv(dfAll, file=file.path(RESULTS_DIR, "deAnnotated_check.csv"))
# end of de.R script
